!NADCsubroutine SwanComputeForce ( fx, fy, ac2, dep2, hs, spcsig, spcdir )
...
!NADC    real, dimension(nverts), intent(in)         :: hs     ! significant wave height
...
!
    ...
    real, dimension(nverts)               :: hs       ! help array to store significant wave height
    !
...
!
    ...
    syy = 0.
    !
    ! move the depths and heights to their own arrays
    do ivert = 1, nverts
       dep2(ivert) = compda(ivert,jdp2)
       hs  (ivert) = compda(ivert,jhs)
    enddo
    !
